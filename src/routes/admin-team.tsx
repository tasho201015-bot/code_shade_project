...
import { Plus, Pencil, Trash2, Copy, Eye, EyeOff, Star, GripVertical, Upload, Search, LayoutGrid, List as ListIcon, X, Settings as SettingsIcon } from "lucide-react";
...
export const Route = createFileRoute("/admin-team")({
  beforeLoad: async () => {
    ...
    if (!session) throw redirect({ to: "/login", search: { redirect: "/admin-team" } });
...
function AdminTeamPage() {
  ...
  const duplicate = async (m: TeamMember) => {
    const copy = {
      ...m,
      name: `${m.name} (Copy)`,
      slug: slugify(`${m.name}-copy-${Date.now()}`),
      is_featured: false,
    };
      ...
      <div className="pt-32 pb-32 max-w-7xl mx-auto px-6 lg:px-10">
        <div className="flex items-end justify-between flex-wrap gap-4">
          <div>
            ...
            <h1 className="font-display text-5xl md:text-6xl mt-2">Team management</h1>
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setEditing({ ...emptyDraft })} className="inline-flex items-center gap-2 bg-noir text-cream px-5 py-2.5 text-xs uppercase tracking-luxe hover:bg-noir/90 transition-colors">
