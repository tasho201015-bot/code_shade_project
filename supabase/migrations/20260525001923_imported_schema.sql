...
-- seed from existing distinct categories on products
INSERT INTO public.categories (name, slug, sort_order)
SELECT
  initcap(replace(c, '-', ' ')) AS name,
  c AS slug,
  row_number() OVER (ORDER BY c) AS sort_order
FROM (SELECT DISTINCT category AS c FROM public.products WHERE category IS NOT NULL) s
...
-- on delete, move products to uncategorized
...
-- prevent deleting the fallback itself
